...
`app.json` also carries `expo.android.versionCode`, which must be bumped with
the version: `major*1000000 + minor*1000 + patch` (0.2.98 → `2098`, 0.2.99 →
`2099`, 0.3.0 → `3000`). Android refuses to install an APK whose versionCode is
lower than the installed one, and the in-app updater relies on each release
being higher. The test failure prints the exact value to write.
