...
const checks: Array<[string, boolean]> = [
  ...
  ['runner-local signing material is always removed', workflow.includes('if: ${{ always() }}') && signingScript.includes("security', ['delete-keychain'") && signingScript.includes("fs.rmSync(temp")],
  ['a successful TestFlight upload retains the remote signing assets Apple validates asynchronously', workflow.indexOf("IOS_RETAIN_REMOTE_SIGNING_ASSETS=true") > workflow.indexOf('xcrun altool --upload-app') && signingScript.includes("process.env.IOS_RETAIN_REMOTE_SIGNING_ASSETS === 'true'")],
  ['failed or non-upload builds still revoke unused remote signing assets', signingScript.includes("if (profileId) await api('DELETE', `/profiles/") && signingScript.includes("if (certificateId) await api('DELETE', `/certificates/")],
  ['archive distribution identity and team are verified before export', workflow.indexOf("Print :ApplicationProperties:SigningIdentity") < workflow.indexOf('xcodebuild -exportArchive') && workflow.includes('TeamIdentifier=$APPLE_TEAM_ID') && workflow.includes('"iPhone Distribution:"*')],
