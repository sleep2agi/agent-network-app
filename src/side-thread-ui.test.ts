...
// 0.2.105: BTW 按钮/格子默认隐藏(chat-entry-flags.ts);格子定义与分派仍在,改回 true 即恢复。
check('+ 菜单 BTW 格子仍定义但受 SHOW_BTW_ENTRY 控制', plusPanel.includes("a11y: '新建 BTW 旁路线程'") && plusPanel.includes('env.btwEntry ?? SHOW_BTW_ENTRY') && chat.includes('不打断、不 steer 当前主任务') && chat.includes('setBtwLaunch(current => ({ id: (current?.id ?? 0) + 1 }))'));
check('/btw 命令仍是 BTW 抽屉的可达入口', chat.includes('parseBtwFirstToken(draft)') && chat.includes('setBtwLaunch(current => ({ id: (current?.id ?? 0) + 1, prompt: parsed.prompt, attachments }))'));
