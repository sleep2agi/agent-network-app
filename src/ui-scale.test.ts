...
// ── 2. density ──
ck('density factors 0.85 / 1 / 1.15', densityFactor('compact') === 0.85 && densityFactor('standard') === 1 && densityFactor('comfortable') === 1.15);
ck('default density: phone/desktop 标准, Android wide 紧凑', defaultDensity(false) === 'standard' && defaultDensity(true) === 'compact');
...
ck('wide with nothing stored → 紧凑 default', uiScale().density === 'compact' && uiScale().densityIsDefault);
ck('spacing mutated in place (same object, md 12 → 10)', spacing === spacingRef && spacing.md === 10, String(spacing.md));
ck('ds() follows (44 → 37)', ds(44) === 37);
ck('ds() floor keeps touch targets (48 → max(41, 44))', ds(48, 44) === 44);
...
ck('fold back to phone with nothing stored → 标准 again', uiScale().density === 'standard' && spacing.md === 12);
...
setUiScaleLayoutWide(true);
ck('summary: 紧凑（宽屏默认）', uiScaleSummary(uiScale(), true).density === '紧凑（宽屏默认）', uiScaleSummary(uiScale(), true).density);
